...
That engine changes the product. A researcher can move from file to viewer to model-guided analysis without first flattening the data into a generic PNG or rebuilding dimensional context by hand. The agent depends on the same engine: an autonomous run reads the same bounded representations a human reviewer sees, so what the model analyzed and what the scientist inspects are the same object.
